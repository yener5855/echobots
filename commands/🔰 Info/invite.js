...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    try {
      ...
      if(user) {
        if(!user.bot) return interaction?.reply({ephemeral: true, content: "<:no:1269533999014084683> You can't Invite a Normal user! **IT MUST BE A BOT**"})
        ...
        let button_support_dc = new MessageButton().setStyle('LINK').setLabel(handlemsg(client.la[ls].cmds.info.invite.buttons.server)).setURL("https://discord.gg/Xrd2D52pRF")
