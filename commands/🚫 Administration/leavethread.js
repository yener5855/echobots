    ...
    try {
      ...
      if(!channel.isThread())
        return message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<:no:1269533999014084683> **\`${channel.name}\` Channel is not a Thread**`)
        ]});
      await channel.leave().then(()=>{
        if(channel.id != message.channel.id){
          message.reply({embeds :[new MessageEmbed()
            .setColor(es.color)
            .setFooter(client.getFooter(es))
            .setTitle(`<a:yes:1269542611408916521> **I left \`${channel.name}\`**`)
          ]});
        }
      }).catch(()=>{
        message.reply({embeds :[new MessageEmbed()
          .setColor(es.wrongcolor)
          .setFooter(client.getFooter(es))
          .setTitle(`<a:yes:1269542611408916521> **I couldn't leave \`${channel.name}\`**`)
        ]});
      })
