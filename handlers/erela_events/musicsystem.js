...
module.exports = client => {
    client.on("interactionCreate", async (interaction) => {
        ...
        if (!member.voice.channel)
            return interaction?.reply({
                content: `<:no:1269533999014084683> **Please join a Voice Channel first!**`,
                ephemeral: true
            })      
            
        var player = client.manager.players.get(interaction?.guild.id);
        if (interaction?.customId != "Join" && interaction?.customId != "Leave" && (!player || !player.queue || !player.queue.current))
            return interaction?.reply({content: "<:no:1269533999014084683> Nothing Playing yet", ephemeral: true})
        ...
        if (player && member.voice.channel.id !== player.voiceChannel)
            return interaction?.reply({
                content: `<:no:1269533999014084683> **Please join __my__ Voice Channel first! <#${player.voiceChannel}>**`,
                ephemeral: true
            })
        ...
        switch(interaction?.customId){
            ...
            case "Autoplay": {
                ...
                interaction?.reply({
                  ...
                  .setColor(ee.color)
                  .setTimestamp()
                  .setTitle(`${player.get(`autoplay`) ? `<a:yes:1269542611408916521> **Enabled Autoplay**`: `<:no:1269533999014084683> **Disabled Autoplay**`}`)
                  .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
                //edit the message so that it's right!
                var data = generateQueueEmbed(client, guild.id)
            ...
            case "Song": {
                ...
                interaction?.reply({
                  ...
                  .setColor(ee.color)
                  .setTimestamp()
                  .setTitle(`${player.trackRepeat ? `<a:yes:1269542611408916521> **Enabled Song Loop**`: `<:no:1269533999014084683> **Disabled Song Loop**`}`)
                  .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
                //edit the message so that it's right!
                var data = generateQueueEmbed(client, guild.id)
            ...
            case "Queue": {
                ...
                interaction?.reply({
                  ...
                  .setColor(ee.color)
                  .setTimestamp()
                  .setTitle(`${player.queueRepeat ? `<a:yes:1269542611408916521> **Enabled Queue Loop**`: `<:no:1269533999014084683> **Disabled Queue Loop**`}`)
                  .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({dynamic: true})))]
                })
                //edit the message so that it's right!
                var data = generateQueueEmbed(client, guild.id)
    ...
    client.on("messageCreate", async message => {
        ...
        if (prefixRegex.test(message.content)) {
            ...
            if (cmd || cmd.length === 0) return// message.reply("<:no:1269533999014084683> **Please use a Command Somewhere else!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)})
            ...
            {
                return// message.reply("<:no:1269533999014084683> **Please use a Command Somewhere else!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)})
            }
        }
        ...
        if (!channel) return message.reply("<:no:1269533999014084683> **Please join a Voice Channel first!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 5000)})
        ...
        if (player && channel.id !== player.voiceChannel) return message.reply(`<:no:1269533999014084683> **Please join __my__ Voice Channel first! <#${player.voiceChannel}>**`).then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)})
...
function generateQueueEmbed(client, guildId, leave){
    ...
    var embeds = [
      ...
      new MessageEmbed()
        ...
        .setDescription(`> *I support <:Youtube:1265089595964391465> Youtube, <:Spotify:1269543430422528073> Spotify, <:soundcloud:825095625884434462> Soundcloud and direct MP3 Links!*`)
    ]
    ...
    if(!leave && player && player.queue && player.queue.current){
        embeds[1].setImage(`https://img.youtube.com/vi/${player.queue.current.identifier}/mqdefault.jpg`)
            ...
            .setAuthor(`${player.queue.current.title}`, "https://images-ext-1.discordapp.net/external/DkPCBVBHBDJC8xHHCF2G7-rJXnTwj_qs78udThL8Cy0/%3Fv%3D1/https/cdn.discordapp.com/emojis/a:Playing_Audio:1269512711918850131.gif", player.queue.current.uri)
