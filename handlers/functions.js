...
function GetUser(message, arg){
  var errormessage = "<:no:1269533999014084683> I failed finding that User...";
...
function GetRole(message, arg){
  var errormessage = "<:no:1269533999014084683> I failed finding that Role...";
  return new Promise(async (resolve, reject) => {
    var args = arg, client = message.client;
    if(!client || !message) return reject("CLIENT IS NOT DEFINED")
    if(!args || args == null || args == undefined) args = message.content.trim().split(/ +/).slice(1);
...
async function swap_pages(client, message, description, TITLE) {
  ...
  let button_back = new MessageButton().setStyle('SUCCESS').setCustomId('1').setEmoji("1269532683248337047").setLabel("Back")
  let button_home = new MessageButton().setStyle('DANGER').setCustomId('2').setEmoji("🏠").setLabel("Home")
  let button_forward = new MessageButton().setStyle('SUCCESS').setCustomId('3').setEmoji('1269532655586644009').setLabel("Forward")
  ...
  collector.on('collect', async b => {
      if(b?.user.id !== message.author.id)
        return b?.reply({content: `<:no:1269533999014084683> **Only the one who typed ${prefix}help is allowed to react!**`, ephemeral: true})
        //page forward
        if(b?.customId == "1") {
